...
def should_clarify(state: ChatState) -> Literal["clarify", "enhance_query"]:
    """라우팅 결과에 따라 clarify 또는 enhance_query로 분기"""
    route_target = state.route.get("target", "")
    route_confidence = state.route.get("confidence", 0.0)
    
    # 신뢰도가 낮거나 clarify 대상인 경우
    if route_target == "clarify" or route_confidence < 0.5:
        return "clarify"
    else:
