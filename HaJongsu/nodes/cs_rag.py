...
try:
    ...
    if config.OPENAI_API_KEY:
        ...
        logger.info("OpenAI client initialized for CS RAG")
    else:
        openai_client = None
...
# PINECONE 벡터 검색 설정
try:
    from utils.pinecone_client import search_faq as pinecone_search_faq
    PINECONE_AVAILABLE = True
    logger.info("PINECONE FAQ search available")
except ImportError:
    PINECONE_AVAILABLE = False
    logger.warning("PINECONE not available, using mock FAQ search")
...
def _search_faq(query: str) -> List[Dict[str, Any]]:
    """FAQ 검색 (PINECONE 우선, 폴백 시 Mock 사용)"""
    
    # PINECONE 벡터 검색 시도
    if PINECONE_AVAILABLE:
        try:
            pinecone_results = pinecone_search_faq(query, top_k=3)
            if pinecone_results:
                ...
                for result in pinecone_results:
                    ...
                    matching_faqs.append(faq_formatted)
                
                logger.info(f"PINECONE FAQ 검색 성공: {len(matching_faqs)}개 결과")
                return matching_faqs
                
        except Exception as e:
            logger.error(f"PINECONE FAQ 검색 실패: {e}")
            # 폴백으로 Mock 검색 계속 진행
    
    # Mock FAQ 검색 (PINECONE 실패 시 또는 사용 불가 시)
    query_lower = query.lower()
    matching_faqs = []
    ...
    logger.info(f"Mock FAQ 검색 완료: {len(matching_faqs)}개 결과")
    return matching_faqs
