...
from utils.database import search_products as db_search_products, get_products_by_category, test_connection
...
class ProductSearchEngine:
    ...
    def _load_or_create_mock_data(self):
        """실제 DB 데이터 로드 또는 폴백 Mock 데이터 생성"""
        # 1차 시도: 실제 데이터베이스에서 로드
        try:
            if test_connection():
                self.product_data = self._load_from_database()
                if self.product_data:
                    logger.info(f"실제 DB에서 {len(self.product_data)}개 상품 로드 성공")
                    self._build_search_index()
                    return
                else:
                    logger.warning("DB 연결은 성공했지만 상품 데이터가 없음")
            else:
        ...
        if os.path.exists(index_file):
            try:
                ...
                logger.info(f"캐시에서 {len(self.product_data)}개 상품 로드")
                return
            except Exception as e:
                logger.warning(f"캐시 데이터 로드 실패: {e}")
        
        # 3차 시도: 폴백 Mock 데이터 생성
        logger.warning("폴백 Mock 데이터를 생성합니다")
        self.product_data = self._create_fallback_data()
        self._build_search_index()
        self._cache_data()
        
    def _load_from_database(self) -> List[Dict[str, Any]]:
        """데이터베이스에서 실제 상품 데이터 로드"""
        try:
            # 모든 상품 데이터 가져오기
            ...
            SELECT 
                ...
                COALESCE(p.origin, '국산') as origin,
                CAST(COALESCE(s.stock, 100) as SIGNED) as stock,
                CASE 
                    WHEN i.organic = 'TRUE' THEN 1
                    ELSE 0
                END as organic,
                COALESCE(c.category, 1) as category_id
            FROM product_tbl p
            ...
            products = []
            for row in results:
                category_name = category_map.get(row.get('category_id', 1), '기타')
                
                # 유기농 키워드 추가
                organic_text = "유기농 " if row.get('organic') else ""
                ...
                product = {
                    ...
                    'organic': bool(row.get('organic')),
                    'search_text': f"{organic_text}{row['name']} {row.get('origin', '')} {category_name}"
                }
            ...
            logger.error(f"DB 상품 데이터 로드 실패: {e}")
            return []
    
    def _create_fallback_data(self) -> List[Dict[str, Any]]:
        """폴백용 Mock 데이터 생성 (확장된 버전)"""
        return [
            {
                'name': '유기농 사과',
                'price': 3000.0,
    ...
    def _cache_data(self):
        ...
        try:
            ...
            logger.info("Product data cached successfully")
        except Exception as e:
            logger.warning(f"Failed to cache product data: {e}")
